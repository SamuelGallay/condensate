...
pub fn get_from_gpu(buffer: &Buffer<Cplx>) -> Array2<Complex64> {
    ...
    let temp = unsafe { std::mem::transmute::<Array2<Cplx>, Array2<Complex64>>(cpu_data) };
    return temp;
}
...
pub fn init(n: usize, eps0: f64, l: f64) -> Array2<Cplx> {
    ...
    return unsafe { std::mem::transmute::<Array2<Complex64>, Array2<Cplx>>(normalized) };
}

pub fn vector_field(n: usize, l: f64) -> Array2<Cplx> {
    let mut a = Array2::<Complex64>::zeros((n, n));
    let dx = l / (n as f64);
    for i in 0..n {
        for j in 0..n {
    ...
    return unsafe { std::mem::transmute::<Array2<Complex64>, Array2<Cplx>>(a) };
}
