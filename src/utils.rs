...
pub fn printmax(buffer: &Buffer<Complex<f32>>, name: &str) -> () {
    let m = max(&get_from_gpu(&buffer).mapv(Complex32::norm));
    println!("Max {} : {}", name, m);
}

pub fn image_from_array(cpu_data: &Array2<f32>) -> ImageBuffer<image::Rgb<u8>, Vec<u8>> {
...
pub fn plot_array(cpu_data: &Array2<f32>, name: &str) -> () {
    let imgbuf = image_from_array(cpu_data);
    imgbuf.save(name).unwrap();
}

pub fn plot_from_gpu(a: &array::Array<Complex32>, name: &str) -> () {
